...
import { createBranchExpenditureRequest } from '../redux/slices/expenditureSlice'

const BranchExpenditure = () => {
    const dispatch = useDispatch()
    const navigate = useNavigate()
    const {loading} = useSelector(state=>state.branch)
    const [details,setDetails] = useState({amount:'',reason:''})
    ...
    const handleSubmit = (e) =>{
        ...
        dispatch(createBranchExpenditureRequest(data))
    }

  return (
    <div className="p-6 bg-white rounded shadow-md max-w-xl mx-auto mt-12 mb-6">
      <h2 className="text-xl font-bold mb-4">Enter Expenditure</h2>
...
export default BranchExpenditure;
