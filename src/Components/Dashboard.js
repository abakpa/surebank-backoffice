...
const Dashboard = () => {
  ...
  .map((branch) => ({ label: branch.name, value: branch._id }));
    ...
    const {
      ...
      const newsbpackage = getSBPackageCount(sbpackage)
      const newsbpackageinsufficient = getSBPackageInsufficientCount(sbpackage)
...
<div className="p-3 sm:p-6">
  ...
  <div className="dashboard-card-grid-divider dashboard-card-grid-divider-admin grid grid-cols-2 lg:grid-cols-4 gap-1.5 sm:gap-3">
      {/* Card 4 - Yellow */}
  ...
  <div className="relative p-1.5 sm:p-3 rounded-lg shadow-md ring-1 ring-white/70 bg-rose-200">
    ...
    <form className="flex flex-col gap-1 mt-1">
      <Select2
        ...
        options={branchOptions}
        value={branchId24}
        onChange={(selectedId) => setBranchId24(selectedId)}
      />
      <DashboardDateRangeFields rangeKey="date24" dateRanges={dateRanges} setDateRanges={setDateRanges} />
    </form>
  </div>
   {/* Card 7 - Pink */}
